...
> **v3.5 — physical-accuracy pass:** the optical model went from plausible to **matchmove-trustworthy**. Distortion is now the diagonally-normalized **3DE model** (Radial-Standard spherical + per-axis Anamorphic-Standard) in the correct direction — portable to/from 3DE / Nuke — and the render and the ST-map bake share it byte-identically (the bake round-trips to sub-pixel). Exposure is physically metered (`UsdGeomCamera.exposure` from T-stop / shutter / ISO / fps); DoF uses the geometric f-number; FOV is per-format and squeeze-aware (six fabricated/impossible sensor-format numbers corrected); focus / stop / exposure are USD time-sampled so a focus rack actually racks the rendered DOF. The pupil gained **cat's-eye + mechanical vignetting** (with automatic stop-down recovery), a **polygonal iris + blade curvature**, and **spherical-aberration apodization**. Every optical change is verified to ~1e-8 against a proven Python oracle (round-trip + analytic-Jacobian == finite-difference → hand-mirror to VEX → numerical match in a live Houdini attribwrangle), with two live-Karma probes grounding the NDC and aperture-sample conventions. Coefficient *values* remain heuristic — the model is now ready to ingest measured lens-grid data. (Accuracy roadmap: Tiers 0–2 of 6 shipped.)
...
- **Karma lens shader VOP** — `cinema_lens_shader`, compiled from `vex/include/karma_cinema_lens.vfl` by `vcc -O vop` at build time. dn-normalized **3DE distortion** (Radial-Standard + per-axis Anamorphic-Standard, matchmove-portable, distorted→undistorted direction) + focus-dependent anamorphic squeeze applied as a projection rescale + **cat's-eye / mechanical-vignette** pupil clip + **polygonal iris** + **spherical-aberration apodization** + entrance-pupil ray origin, on the documented Karma lens-shader contract (matches Karma's standard projection when zeroed; NDC→dn and unit-disc aperture conventions empirically probed on 21.0.765). Karma CPU verified; XPU lens-shader support unverified — the (dn-consistent, round-tripping) STMap AOV stays the renderer-agnostic route.
...
- [x] **Accuracy Tier 0 — wire-through + framing** — physically-metered `UsdGeomCamera.exposure` (T-stop/shutter/ISO/fps); per-format active-sensor FOV + corrected body specs (fabricated/impossible modes dropped); geometric f-number for DoF; USD time-sampling so focus/DoF/exposure animate; squeeze-aware FOV; centered shutter.
- [x] **Accuracy Tier 1 — distortion spine** — dn-normalized 3DE Radial-Standard (spherical) + Anamorphic-Standard (per-axis cos2φ/cos4φ), correct direction, portable to 3DE/Nuke; the render and the Copernicus ST-map bake share the same VEX (`co_stmap_pixel`) and round-trip to sub-pixel.
- [x] **Accuracy Tier 2 — the pupil** — cat's-eye + mechanical vignetting (field-offset vesica clip; darkening via sample rejection; automatic stop-down recovery); polygonal iris + blade curvature; spherical-aberration apodization on a `tint` weight. Empirically grounded by `scripts/probe_pupil.py`.
- [ ] **Accuracy Tiers 3–6** — per-body OCIO/ACES color + rolling shutter + dual-native-ISO noise; cos⁴ / halation / veiling-glare + chromatic aberration; 1/f handheld + support archetypes (Steadicam/gimbal/dolly/crane); ground-truth validation harness + lens-schema v5 (structured provenance, measured lens-grid coefficients). Prioritized by impact × feasibility.
- [ ] **Measured lens-grid data** — the accuracy pass made the model correct + portable; the distortion/breathing/vignette *values* are still heuristic (`_provenance`-tagged). Real per-lens 3DE solves are the remaining data task.
...
| Version | Notes |
|---|---|
| v3.5 | Physical-accuracy pass (accuracy roadmap Tiers 0–2). **Tier 0:** physically-metered `UsdGeomCamera.exposure`; per-format squeeze-aware FOV + corrected body specs (six fabricated/impossible mode numbers fixed); geometric f-number for DoF; USD time-sampling so a focus rack animates the rendered DOF; centered shutter. **Tier 1:** dn-normalized 3DE distortion (Radial-Standard + Anamorphic-Standard, correct direction, 3DE/Nuke-portable) shared byte-identically by the render and the round-tripping ST-map bake. **Tier 2:** cat's-eye + mechanical vignetting (stop-down recovery), polygonal iris + blade curvature, spherical-aberration apodization. Method throughout: Python oracle (round-trip + analytic-Jacobian == finite-difference) → hand-mirror to VEX → numerical match in a live Houdini attribwrangle (~1e-8), plus live-Karma NDC + aperture-sample probes. Coefficient values remain heuristic; the model is now ready for measured lens-grid data. |
